...
export default function Geolocalizacao() {
    const [localizacao, setLocalizacao] = useState(null); // Estado para armazenar a localização do usuário
        ...
        script.onload = () => { // Executa quando o script é carregado
            ...
            navigator.geolocation.getCurrentPosition(
                (pos) => { // Se conseguir obter a posição
                    ...
                    setLocalizacao(coords); // Atualiza o estado com as coordenadas

                    // Inicializa o mapa do Mapbox
                    // 'mapboxgl' é uma variável global adicionada pelo script do Mapbox
    ...
    return (
        <div className={styles.container}> {/* Container principal com estilos */}
            ...
            {localizacao && ( // Se a localização estiver disponível
                <div className={styles.info}> {/* Exibe informações da localização */}
                    <h2>📍 Sua Localização</h2>
                    <p>Latitude: {localizacao[1].toFixed(2)}</p> {/* Mostra latitude formatada */}
                    <p>Longitude: {localizacao[0].toFixed(2)}</p> {/* Mostra longitude formatada */}
                </div>
            )}
        </div>
    );
